...
function displayItems(){
    ...
    var desserts = document.getElementById('dessert')
    ...
    const sandwichesData= foodItem.filter((item)=>item.category=='Sandwiches');
    ...
    const wrapsData= foodItem.filter((item)=>item.category=='Wraps');
...
document.getElementById('checkout-button').addEventListener('click', showReceipt);
...
function generateReceiptText() {
    const receiptContent = document.querySelector('.receipt-items');
    receiptContent.innerHTML = ''; // Clear existing content

    let total = 0;

    // Additional information
    const invoiceId = 'Invoice ID: #12345';
    const cafeName = 'Cafe Name: Your Cafe';
    const gstNumber = 'GST Number: 1234567890';
    ...
    for (const item of cartData) {
        ...
        itemRow.classList.add('receipt-row');
        itemRow.innerHTML = `
            <div class="receipt-column">${item.name}</div>
            <div class="receipt-column">${item.quantity}</div>
            <div class="receipt-column">$${itemTotal}</div>
        `;
        receiptContent.appendChild(itemRow);
        total += parseFloat(itemTotal);
    }

    // Add a row for the total amount
    const totalRow = document.createElement('div');
    totalRow.classList.add('receipt-row', 'total-row');
    totalRow.innerHTML = `
        <div class="receipt-column"></div>
        <div class="receipt-column"><strong>Total</strong></div>
        <div class="receipt-column">$${total.toFixed(2)}</div>
    `;
    receiptContent.appendChild(totalRow);
...
function printReceipt() {
    const receiptContent = document.querySelector('.modal-content');
...
// Close the receipt modal when clicking outside the modal
